...
class ToolRepository:
    ...
    def create_tool(self, data: ToolInputSchema) -> ToolOutputSchema:
        provider = Tool(**data.model_dump(exclude_none=True))
        self.session.add(provider)
        self.session.commit()
        self.session.refresh(provider)
        return ToolOutputSchema.from_orm(provider)

    def get_tool_details_by_id(self, _id: int) -> ToolOutputSchema:
        return ToolOutputSchema.from_orm(self.session.query(Tool).filter_by(id=_id).first())
    ...
    def get_tools(self) -> List[ToolOutputSchema]:
        return [ToolOutputSchema.from_orm(provider) for provider in self.session.query(Tool).all()]
